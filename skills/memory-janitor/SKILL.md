---
name: memory-janitor
description: Automated cleanup and compaction of memory directory plus system-level disk cleanup. Cleans GEP prompts, deduplicates candidates.jsonl, compacts memory_graph.jsonl, archives old daily notes, and optionally cleans npm cache, __pycache__, journal logs, and stale skill logs. Triggers on "clean memory", "compact memory", "memory health", "memory stats", "archive notes", "disk cleanup", "free disk space", "clean cache", "dedup candidates".
---
...
```bash
# Full memory cleanup (prompts + graph + daily note archive + candidates dedup)
...
5. **Stats Report**: Shows memory directory size breakdown by category
